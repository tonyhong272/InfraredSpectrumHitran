...
class TabDialog(QtGui.QDialog): 
    def __init__(self): 
        ...
        self.mixedBroadening = QtGui.QCheckBox('Use Mixed Broadening')
        self.mixedBroadening.setChecked(True)
        ...
        conditionLayout2.addWidget(self.mixedBroadening)
        ...
        rangeLayout.addWidget(ylowLabel)
        ...
        self.plotButton = QtGui.QPushButton("Figure Update") 
        self.plotButton.pressed.connect(self.PlotStatus) 
        self.plotButton.clicked.connect(self.Update) 
        self.saveButton = QtGui.QPushButton("Save Figure") 
        self.saveButton.clicked.connect(self.saveFig) 
        self.exportButton = QtGui.QPushButton("Export Data") 
        ...
        seperateLayer2 = QtGui.QHBoxLayout()
        seperateLayer2.addWidget(QtGui.QLabel('\n----------------Add/Remove Gas-----------------'))
        ...
        self.pathlengthBox = QtGui.QLineEdit('1')
        ...
        #########################  add different gas plot
        AddGasLayout = QtGui.QHBoxLayout()
        GasSelectionLabel = QtGui.QLabel('Select Gas')
        self.GasSelection = QtGui.QComboBox(self)
        moleculeSpecies = molecules().moleculeSpecies()
        for mol in moleculeSpecies:
            self.gasCombo.addItem(mol)
            self.GasSelection.addItem(mol)
        self.gasCombo.setMaxVisibleItems(50)
        self.GasSelection.setMaxVisibleItems(50)
        GasPercentageLabel =  QtGui.QLabel('Percent in Air')
        self.percentageBox = QtGui.QLineEdit('100')
        AddGasLayout.addWidget(GasSelectionLabel)
        AddGasLayout.addWidget(self.GasSelection)
        AddGasLayout.addWidget(GasPercentageLabel)
        AddGasLayout.addWidget(self.percentageBox)
        AddLayout = QtGui.QHBoxLayout()
        self.addButton = QtGui.QPushButton("Add Plot") 
        self.addButton.pressed.connect(self.addGasStatus)
        self.addButton.clicked.connect(self.AddGas)
        AddLayout.addWidget(self.addButton)
        RemGasLayout = QtGui.QHBoxLayout()
        GasSelectionLabel1 = QtGui.QLabel('Select Gas')
        self.GasSelection1 = QtGui.QComboBox(self)
        RemGasLayout.addWidget(GasSelectionLabel1)
        RemGasLayout.addWidget(self.GasSelection1)
        RemLayout = QtGui.QHBoxLayout()
        self.remButton = QtGui.QPushButton("Remove Plot") 
        self.remButton.clicked.connect(self.RemGas)
        RemLayout.addWidget(self.remButton)
        #########################  put layout together
...
#        sideLayout2.addWidget(separatorLabel5)
        ...
        sideLayout.addLayout(seperateLayer2)
        sideLayout.addLayout(AddGasLayout)
        sideLayout.addLayout(AddLayout)
        sideLayout.addLayout(RemGasLayout)        
        sideLayout.addLayout(RemLayout)
        ...
        self.mol = []
        self.mol.append(self.gasCombo.currentText()+':'+self.percentageBox.text()+'%')
        self.parameters = [self.check_um,self.xlowBox,self.xhighBox,self.currUnit,self.ylowBox,self.yhighBox, self.yaxis, self.mol, self.pathlengthBox,self.tempBox,self.pressureBox, self.yautoscale] 
#==============================================================================
#     [0:self.check_um,1:self.xlowBox,2:self.xhighBox,3:currUnit,4:self.ylowBox,5:self.yhighBox, 
#        6:self.yaxis, 7:self.mol, 8:self.pathlengthBox,9:self.tempBox,10:self.pressureBox,
#        11: self.yautoscale]
#       
#==============================================================================
    def saveFig(self):
        canvas.fig.savefig('./save/image.pdf')
    def addGasStatus(self):
        self.addButton.setText('Loading... Please wait...')
        self.addButton.setStyleSheet("color: rgb(255, 0, 255);")
    def AddGas(self):
        self.GasSelection1.addItem(self.GasSelection.currentText()+':'+self.percentageBox.text()+'%')
        self.GasSelection1.setCurrentIndex(self.GasSelection1.count()-1)
        self.parameters[7].append(self.GasSelection.currentText()+':'+self.percentageBox.text()+'%')
        self.data_raw = getData().read_hitran2012_parfile(molecules().filename(self.GasSelection.currentText()), self.addButton)
        self.data_temp = getData().calculate_hitran_xsec(self.data_raw, wavemin=float(self.waveminBox.text()), wavemax=float(self.wavemaxBox.text()), npts=int(self.nptsBox.text()), units='cm^2', temp=273+float(self.tempBox.text()), pressure=float(self.pressureBox.text()), concentration = self.percentageBox, mixedBroadening = self.mixedBroadening.isChecked())    
        self.dataTotal.append(self.data_temp)
        DataHolder().update(self.dataTotal,self.parameters)
        self.addButton.setText('Add Plot')
        canvas.update(self.dataTotal,self.parameters) 
        self.addButton.setStyleSheet("color: rgb(0, 0, 0);")
        print(self.parameters[7])
    def ExportData(self):
        np.savetxt('./data/examp_output_x.txt', self.data[0], delimiter = ',')
        density = float(self.parameters[10].text())*101325/1.38E-23/(float(self.parameters[9].text())+273)*1E-6 #measure in cm-3
        DataAbsorption = np.exp(-self.data[1]*density*float(self.parameters[8].text()))
        np.savetxt('./data/examp_output_y.txt', DataAbsorption, delimiter = ',')
    def RemGas(self):
        curInd = (self.GasSelection1.currentIndex())
        self.GasSelection1.removeItem(curInd)
        self.dataTotal.pop(curInd)
        self.parameters[7].pop(curInd)
        DataHolder().update(self.dataTotal,self.parameters)
        canvas.update(self.dataTotal,self.parameters) 
    ...
    def Update(self): 
        canvas.update(self.dataTotal,self.parameters) 
        self.plotButton.setText("Figure Update")
    ...
    def LoadData(self):
        ...
        self.data = getData().calculate_hitran_xsec(self.data_raw, wavemin=float(self.waveminBox.text()), wavemax=float(self.wavemaxBox.text()), npts=int(self.nptsBox.text()), units='cm^2', temp=273+float(self.tempBox.text()), pressure=float(self.pressureBox.text()), concentration = self.concentrationBox, mixedBroadening = self.mixedBroadening.isChecked())    
        self.parameters[7] = []
        self.parameters[7].append (self.gasCombo.currentText()+':'+self.concentrationBox.text()+'%')
        self.GasSelection1.clear()
        canvas.FirstPlot(self.data, self.parameters)
        self.dataTotal =[]
        self.dataTotal.append(self.data)
        self.GasSelection1.addItem(self.selectMol+':'+self.concentrationBox.text()+'%')
        self.plotButton.setDefault(True)
        ...
        self.loadBox.setText('Finished Loading' + molecules().filename(self.selectMol) + '\ndata loaded and plotted successfully')
...
class DataHolder(): 
    ...
    def update(self,dataTotal,parameters): 
        self.ax.cla()
        self.yaxis = parameters[6].currentText()
        ...
        if self.check_um.isChecked():
            ...
            parameters[3] = 1
            self.ax.set_xlabel('Wavelength (um)')
            for mol in (parameters[7]):
                data = dataTotal[parameters[7].index(mol)]
                self.ax.plot(data[0], self.ProcessYaxis(data[1],parameters), label=mol)  
        else:
            unit_um = False
            if parameters[3] == 1:
                self.xmin1 = self.xmin
            ...
            for mol in (parameters[7]):
                data = dataTotal[parameters[7].index(mol)]
                self.ax.plot(10000/data[0], self.ProcessYaxis(data[1],parameters), label=mol)  
        self.ax.legend(bbox_to_anchor=(1, 1))
        self.ax.set_ylabel(self.yaxis)
        self.ax.set_title("Infrared Spectrum - "+self.yaxis)        
...
class getData():
    ...
    def calculate_hitran_xsec(self,data, wavemin=None, wavemax=None, npts=20001, units='cm^2', temp=296.0, pressure=1.0, concentration=100, mixedBroadening= True):
#==============================================================================
#     '''
...
#==============================================================================
        ...
        if mixedBroadening == True:
            ...
            linewidths = array(data['gamma-self'][okay])
